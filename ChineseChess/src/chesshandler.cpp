...
void ChessHandler::connStateChanged(bool status)
{
    ...
    if (status == true)
    {
        tipMsg = g_gameSettings.getServerOrClient() == SERVER_SIDE ? QStringLiteral("客户端已连接。") : QStringLiteral("服务器已连接。");
        QMessageBox::about(NULL, QStringLiteral("提示信息"), tipMsg);
    }
    else
    {
        tipMsg = g_gameSettings.getServerOrClient() == SERVER_SIDE ? QStringLiteral("客户端已断开。") : QStringLiteral("服务器已断开。");
        tipMsg.append(QStringLiteral("游戏将进入自由模式"));
        QMessageBox::about(NULL, QStringLiteral("提示信息"), tipMsg);
